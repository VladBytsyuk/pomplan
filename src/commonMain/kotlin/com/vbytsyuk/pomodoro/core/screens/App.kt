...
class App(
    ...
    private val signIn: Screen<SignIn.State, SignIn.Action, SignIn.Effect> =
        SignIn(authApi, googleSignIn, appleSignIn, twitterSignIn).apply {
            controller.subscribeOnState(appSubscriber) {
                this@App.controller.setAction(Action.ChangedSignInState(it))
            }
        }
    private val pomodoro: Screen<Pomodoro.State, Pomodoro.Action, Pomodoro.Effect> =
        Pomodoro(settingsRepository).apply {
            controller.subscribeOnState(appSubscriber) {
                this@App.controller.setAction(Action.ChangedPomodoroState(it))
            }
        }
    private val statistics: Screen<Statistics.State, Statistics.Action, Statistics.Effect> =
        Statistics().apply {
            controller.subscribeOnState(appSubscriber) {
                this@App.controller.setAction(Action.ChangedStatisticsState(it))
            }
        }
    private val settings: Screen<Settings.State, Settings.Action, Settings.Effect> =
        Settings().apply {
            controller.subscribeOnState(appSubscriber) {
                this@App.controller.setAction(Action.ChangedSettingsState(it))
            }
        }
